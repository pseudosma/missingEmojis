...
@implementation emojiViewControlleriPhone
...
/* explicitly defined gesture recognizers are added to each view*/
...
/* This method creates views and, within those, image views holding the image array's images. Creating them this way takes advantage of the performance benefits of instantiating the imageViews with images. This process is done over three seperate methods.*/  
    
    view = [[UIView alloc] initWithFrame:CGRectMake((45 * arrayCount) + xSpacing, 0, 40, 40)];
...
/* The following if/then helps maintain proper spacing across scrollViewA */
...
-(void)makeImageViewsAndParentViewsSecondRow:(UIImageView *)imageView :(UIView *)view{
    ...
    view = [[UIView alloc] initWithFrame:CGRectMake((45 * secondArrayCount) + xSpacing, 55, 40, 40)];
    imageView = [[UIImageView alloc] initWithImage:[imageArray objectAtIndex:arrayCount]];
    ...
    if (secondArrayCount == 26) {
        xSpacing = 25;
    }
    if (secondArrayCount == 35) {
        xSpacing = 33;
    }
...
-(void)makeImageViewsAndParentViewsThirdRow:(UIImageView *)imageView :(UIView *)view{
    ...
    view = [[UIView alloc] initWithFrame:CGRectMake((45 * thirdArrayCount) + xSpacing, 110, 40, 40)];
    imageView = [[UIImageView alloc] initWithImage:[imageArray objectAtIndex:arrayCount]];
...
-(void)viewDidLoad
{
/* create the imageArray first and give a value to xSpacing*/
    
    xSpacing = 5;
    [self makeImageArray];
    
    dimensions = CGRectMake(0, 273, 320, 150);
    ...
    scrollViewA.contentSize = CGSizeMake(1920, 150);
    ...
    secondArrayCount = 0;
    xSpacing = 5;
/* Three seperate methods are used for creating the emojis in the scrollview - one for each row. */    
    do {
...
/* This method works to set the UIPageControl in the main view controller to value of emojiViewController's pageNumber property. The event is communicated via a notification.*/
    pageNumber = 0;
    ...
    if (scrollView.contentOffset.x > 959) {
        pageNumber = 3;
    }
    if (scrollView.contentOffset.x > 1279) {
        pageNumber = 4;
    }
    if (scrollView.contentOffset.x > 1599) {
        pageNumber = 5;
    }
...
/* This method works in conjunction with the main view controller and stageViewController's quickCopyEmoji methods*/      
...
/* This method works in conjuction with the main view controller's copButtonPushed and the stageViewController's copButton methods. */ 
   arrayIndex = gestureRecognizer.view.tag;
